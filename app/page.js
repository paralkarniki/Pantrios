			...
			<div style={{
				...
				{FEATURES.map((f) => (
					<div key={f.title} style={{
						...
						<div style={{ fontSize: '1.5rem', marginBottom: '.7rem' }}>{f.icon}</div>
						<h3 style={{ margin: '0 0 .35rem', fontSize: '1.05rem', color: '#1c1917' }}>{f.title}</h3>
						<p style={{ margin: 0, color: '#78716c', lineHeight: 1.6, fontSize: '.95rem' }}>{f.desc}</p>
					</div>
				))}
			</div>

			<div style={{ marginTop: '3.5rem', display: 'grid', gap: '.9rem', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
				{STEPS.map((s) => (
					<div key={s.n} style={{
						background: 'rgba(255,255,255,0.72)',
						borderRadius: 18,
						...
						<div style={{ fontWeight: 800, color: '#d97706', marginBottom: '.35rem' }}>{s.n}</div>
						<div style={{ fontWeight: 700, color: '#1c1917', marginBottom: '.25rem' }}>{s.label}</div>
						<div style={{ color: '#78716c', lineHeight: 1.6, fontSize: '.95rem' }}>{s.sub}</div>
					</div>
				))}
			</div>
			</section>
