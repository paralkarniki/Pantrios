  ...
  if (estimate > target + 90) {
    for (let i = 0; i < adjusted.length; i += 1) {
      adjusted[i] = adjusted[i]
        ...
        .replace(/\b1\.5\s*cups?\b/i, '1 cup')
        .replace(/\b300\s*g\b/i, '220 g')
        .replace(/\b250\s*g\b/i, '180 g')
    }
  }

  estimate = estimateRecipeCalories({ ingredients: adjusted })

  if (estimate > target + 80) {
    ...
    removablePriority.forEach((token) => {
      const idx = adjusted.findIndex((line) => line.toLowerCase().includes(token.toLowerCase()))
      if (idx >= 0) {
        const next = adjusted.filter((_, i) => i !== idx)
        const nextEstimate = estimateRecipeCalories({ ingredients: next })
        if (nextEstimate >= target - 120) {
